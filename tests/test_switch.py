...
async def test_power_switch(
    ...
    """Test on/off power switch."""
    entity_id = "switch.fake_brand_homeappliance_powerstate"
    mock_appliance.entities["Test.PowerState"].__init__(
        EntityDescription(
            uid=200,
    ...
    await mock_appliance.entities["Test.PowerState"].update({"value": 1})
    ...
    mock_appliance.session.send_sync.assert_awaited_once_with(
        Message(
            resource="/ro/values",
            action=Action.POST,
            data={"uid": 200, "value": 1},
        )
    )


async def test_standby_switch(
    hass: HomeAssistant,
    mock_appliance: MockAppliance,
    patch_entity_description: None,  # noqa: ARG001
) -> None:
    """Test on/standby power switch."""
    entity_id = "switch.fake_brand_homeappliance_powerstate"
    mock_appliance.entities["Test.PowerState"].__init__(
        EntityDescription(
            uid=200,
            name="Test.PowerState",
            enumeration={"2": "On", "3": "Standby"},
            available=True,
            access=Access.READ_WRITE,
        ),
        mock_appliance,
    )
    await mock_appliance.entities["Test.PowerState"].update({"value": 3})
